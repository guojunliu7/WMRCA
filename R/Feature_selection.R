...
metabolism <- function(Data,type="Lipid"){
  if(type=="Lipid"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"Lipid"]),]
  }else if(type=="Amino_acid"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"Amino_acid"]),] 
  }else if(type=="Carbohydrate"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"Carbohydrate"]),]
  }else if(type=="Energy"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"Energy"]),]
  }else if(type=="Nucleotide"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"Nucleotide"]),]
  }else if(type=="TCA_cycle"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"TCA_cycle"]),]
  }else if(type=="Vitamin_cofactor"){
    Data=Data[trim(row.names(Data)) %in% trim(pathway_all[,"Vitamin_cofactor"]),]
  }
  Data
}
